...
function postRecord() {
  try {
    ...
    socket.emit('newRecord', recordObject)
...
const chatUserEmail = document.querySelector('#user-email')
const chatUserMessage = document.querySelector('#user-message')
const renderedMessages = document.querySelector('#rendered-messages')
...
async function renderMessages(messages) {
    try {
    const html = await messages.map((messageInfo) => {
      return `<div>
    ...
    renderedMessages.innerHTML = html
  } catch(err) {
    console.log(`There was an error retrieving the messages, error: ${err}`)
  }
}
